...
async function performTransaction (appointment, type) {
    ...
    if (centralNodeConnection) {
        try {
            ...
            //} else if (type === 'UPDATE') {
                dbFuncs.updateAppointment(appointment, centralNodeConnection);
            //} else {
    ...
    if (nodeConnection) {
        try {
            ...
            //} else if (type === 'UPDATE') {
                updateAppointmentError(appointment, nodeConnection);
            //} else {
...
const case4Fail = async() =>{
    ...
    try{   
        ...
        const appoint ={
            ...
            pxid: "delete",
            clinicid: "delete",
            regionname:"mapupunta sa VisMindanao",
            status: "random",
            type:"Inpatient " + Math.random(),
            timequeued: new Date().toISOString().slice(0, 19).replace('T', ' ')
        }
       
        
        await performTransaction(appoint, "UPDATE")
        await replicateOtherNodes(); // replicate again
